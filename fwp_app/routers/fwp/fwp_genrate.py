...
def credit_card(pdf,json_data,c_MoneyS,money_signData):
    ...
    def get_rect_h(rem,df,i):
        ...
        annual_fee = list(df['annual_fee'].iloc[i].split(' '))
        annual_fee_text = ''
        annual_fee_rem = []
        for k in range(len(annual_fee)):
            annual_fee_text = annual_fee_text+' '+annual_fee[k]
            if mm2PX(pdf.get_string_width(annual_fee_text)) > 390:
                annual_fee_text = annual_fee_text.replace(annual_fee[k],'')
                annual_fee_rem = annual_fee[k:]
                break
        pdf.multi_cell(px2MM(390), px2MM(32),annual_fee_text,align='L')
        if not annual_fee_rem ==[]:
            pdf.set_xy(px2MM(146),px2MM(det_y+32)) 
            pdf.multi_cell(px2MM(518), px2MM(32),' '.join(annual_fee_rem),align='L')
    ...
    for i in range(len(df)):
        ...
        pdf.set_font('LeagueSpartan-SemiBold', size=px2pts(24))
        pdf.set_text_color(*hex2RGB('#1A1A1D'))
        pdf.set_xy(px2MM(267),px2MM(det_y)) 
        annual_fee = list(df['annual_fee'].iloc[i].split(' '))
        annual_fee_text = ''
        annual_fee_rem = []
        for k in range(len(annual_fee)):
        ...
        pdf.set_font('LeagueSpartan-SemiBold', size=px2pts(24))
        pdf.set_text_color(*hex2RGB('#1A1A1D'))
        pdf.set_xy(px2MM(255),px2MM(det_y)) 
        eligibility = list(df['eligibility'].iloc[i].split(' '))
        elg_text = ''
        elg_rem = []
        for k in range(len(eligibility)):
        ...
        for k in range(len(best_suited_for)):
            ...
            if mm2PX(pdf.get_string_width(suited_text)) > 340:
                ...
                suited_rem = best_suited_for[k:]
                break
        pdf.set_xy(px2MM(305),px2MM(det_y)) 
        # pdf.multi_cell(px2MM(350), px2MM(32),df['best_suited_for'].iloc[i],align='L')
        pdf.multi_cell(px2MM(350), px2MM(32),suited_text,align='L')
